...
<Card.Img style={{height:"9rem", width:'6rem'}} variant="top" src={item.image} />
<Card.Body>
<Card.Title style={{fontSize:'small'}}>{item.title}</Card.Title>
<Card.Text>
    <per className="text-decoration-line-through">₹{item.price+500}</per>   <br/>       
    ₹{item.price}
</Card.Text>
<Button variant="primary" onClick={()=>nav(`/show/${item._id}`)}>Shopnow</Button>
</Card.Body>
</Card>
))}
   </div>
